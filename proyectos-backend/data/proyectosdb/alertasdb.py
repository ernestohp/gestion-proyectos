...
def lista_usuarios():
    ...
    sql = """
            SELECT email, name, id, horas_min, horas_max, alerta1, alerta2, alerta3, extras
            FROM public."usuario_alerta"
        """
    try:
        cur = conn.cursor()
        cur.execute(sql)
        columns = ('email', 'name', 'id', 'horas_min', 'horas_max', 
                   'alerta1', 'alerta2', 'alerta3', 'extras')
        results = []
...
def insertar_usuario(data):
    ...
    sql = """
            INSERT INTO 
            public."usuario_alerta"(email, horas_min, horas_max, id, name, alerta1, alerta2, alerta3, extras)
