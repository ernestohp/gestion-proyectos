...
export const obtenerAusencias = async(id) => {
  const url1 = Constantes.BASE_END_POINT + '/personas/'+id+'/ausencias'
  const res = await fetch(url1)
  const data = await res.json()
  return data
}
...
export const grabarProyectoDescripcion_AM_PO = async(proyecto) => {
  const requestOptions = {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin':'*' },
    body: JSON.stringify({"proyecto": proyecto})
  };
  const url1 = Constantes.BASE_END_POINT + '/proyecto/actualizar'
  const res = await fetch(url1, requestOptions)
  const data = await res.json()
  return data
}

export const grabarProyectoPlantilla = async(proyecto) => {
  const requestOptions = {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin':'*' },
    body: JSON.stringify(proyecto)
  };
  const url1 = Constantes.BASE_END_POINT + '/proyecto/plantilla'
  const res = await fetch(url1, requestOptions)
  const data = await res.json()
  return data
}