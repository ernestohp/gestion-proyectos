...
export const horasLaborales2 = (d1, d2, arr_ausencias) => {
  var horasTotales = 0
  const dias = diasLaborales(d1, d2);
  horasTotales = dias*8

  arr_ausencias.map((a)=>{
    const dAusencia = getFecha1(a.fecha)
    if (dAusencia.getTime() >= d1.getTime() && dAusencia.getTime() <= d2.getTime()){
      const hours = a.horas
...
//Crear objeto Date desde un formato dd/mm/yyyy
